...
| קובץ | שימוש |
|---|---|
| `store-assets/app-icon-1024.png` | אייקון לאפל (App Store Connect דורש בדיוק 1024×1024, ריבוע מלא בלי שקיפות/פינות מעוגלות — כך נבנה) |
| `icons/icon-512.png` (קיים) | אייקון לגוגל פליי |
...
- [ ] **צילומי מסך אמיתיים** — לפי הרשימה למעלה. עדיין לא הופקו (כלי הצילום בסביבה הזו דורש שהחלונית תוצג בצד שלך; פתחו את `bouzoukifret.vercel.app` בטלפון/DevTools ותצלמו את 6 המסכים).
...
- [ ] **העלאת קבצי גרפיקה ל-Play Console** — האייקון (`store-assets/app-icon-1024.png`/`icons/icon-512.png`), ה-Feature Graphic (`store-assets/play-store-feature-graphic.png`) והצילומים (כשיהיו) צריכים גרירה ידנית לדף "דף האפליקציה בחנות" ← "נכסים חזותיים נפוצים"/"נכסים חזותיים ייעודיים" — כלי האוטומציה לא יכול לגשת לקבצים מקומיים מחוץ לתיקיית השיתוף שלו.
