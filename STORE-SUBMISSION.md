...
## Google Play — סטטוס נוכחי (2026-07-29): בבדיקה אצל Google, במסלול Closed Testing

**כל הנכסים הגרפיים הועלו** (ידנית, ע"י המשתמש): אייקון, Feature Graphic, ו-8 צילומי מסך בדף החנות הראשי. דף החנות מלא לגמרי.

### גילוי קריטי: גישה ל-Production דורשת בדיקה סגורה של 14 יום — לא ניתן לעקיפה

חשבונות מפתח חדשים ב-Google Play (כמו זה) חייבים להריץ **מסלול "בדיקה בקבוצה מוגדרת" (Closed Testing) עם לפחות 12 בודקים, במשך 14 יום רצופים לפחות**, לפני שאפשר בכלל להגיש בקשה לגישה לסביבת הייצור (Production). זו מדיניות רשמית של גוגל למניעת ספאם, לא באג ולא משהו שניתן לזרז. **נבדק ואומת ב-`מרכז השליטה` ← "הגשת בקשה לגישה לסביבת הייצור"**: 0 מתוך 12 בודקים נדרשים כרגע.

### מה נעשה בפועל (2026-07-29)

1. **מסלול "Alpha" (Closed Testing) הוגדר**: הגרסה (versionCode 3, עם האייקון החדש) **קודמה** (promote, לא הועלתה מחדש) ממסלול הבדיקה הפנימית — Play Console מעתיק אוטומטית את אותו AAB + הערות הגרסה, בלי צורך בהעלאה חוזרת.
2. **נתקלנו ותוקנו 3 בעיות אמיתיות בדרך**:
   - טיוטת גרסה שגויה (משגיאת "קוד גרסה כבר בשימוש" מניסיון קידום כפול) נשארה תקועה ב-Alpha וחסמה קידום נקי — נמחקה ידנית לפני ניסיון נוסף.
   - לא נבחרו מדינות/אזורים להפצה במסלול — תוקן ע"י בחירת "כל המדינות/האזורים".
   - **התגלית המשמעותית ביותר**: האפליקציה הייתה מוגדרת ב-Play Console כ**"בתשלום"** (ללא merchant account מחובר) — זו הייתה הסיבה האמיתית לשגיאת "עוד לא ניתן לפרסם את האפליקציה", לא רק המדינות. תוקן ב-`תמחור האפליקציה` ← שונה ל**"חינם"** (תואם לאפליקציה בפועל — אין שום רכישה/IAP).
3. **הגרסה נשמרה בהצלחה** במסלול Closed Testing (Alpha), עם כל הארצות פעילות.
4. **13 השינויים המצטברים נשלחו לבדיקת Google** (`סיכום השינויים לפרסום` ← "שליחת 13 שינויים לבדיקה") — נכנסו ל"בדיקות מהירות" (עד 14 דקות), ואז בדיקה מלאה שיכולה להימשך עד 7 ימים.

### מה נשאר למשתמש בפועל

1. **לגייס 12 בודקים אמיתיים** (חברים/משפחה/תלמידים) עם כתובות Gmail, לשלוח להם קישור הצטרפות למסלול Alpha (מ-`בדיקות בקבוצות מוגדרות` ← לשונית "בודקים"), ולוודא שהם מצטרפים ומתקינים בפועל.
2. **להמתין 14 יום רצופים** עם הקבוצה פעילה (דרישה קשיחה של גוגל, אין דרך לקצר).
3. אחרי שהתנאי מתמלא: לחזור ל-`מרכז השליטה` ← "הגשת בקשה לגישה לסביבת הייצור" ולהגיש בקשה.
4. רק אז אפשר לקדם את הגרסה (עם אותו AAB) למסלול Production ולשלוח לבדיקה הסופית — כל הטפסים (Data Safety, App content, Store listing) כבר מוכנים ולא ידרשו מילוי נוסף.

**הערה טכנית לעתיד**: כל עדכון עתידי ל-AAB (למשל בפעם הבאה שהאייקון או קוד ה-TWA משתנים) חייב `appVersionCode` גבוה יותר מהאחרון ששימש בכל מסלול (כרגע 3).
